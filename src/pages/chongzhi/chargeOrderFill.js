...
export default class ChargeOrderFill extends BaseComponent {
    state = {
        delta: 1,
    ...
    componentDidMount() {
        ...
        const {cart_ids, way, cateType} = navigation.state.params;
        ...
        return (
            <View style={PublicStyles.ViewMax}>
                <ScrollView>
                                    ...
                                    >
                                        <View style={styles.oneItem}>
                                            ...
                                            <View style={styles.oneItemBody}>
                                                <Text style={styles.oneItemBodyTitle}>
                                                ...
                                                <Text
                                                    style={styles.oneItemBodyPrice}>¥{calculate ? (calculate.goods_amount + calculate.pay_freight_fee) : total}</Text>
                                            </View>
                                        </View>
                                    </Item>
                                )) : null
                    ...
                    {cateType && cateType !== 'member' ? <View style={styles.message}>
                        <TextareaItem
                            rows={3}
                            value={message}
                            placeholder={'必填，请填写您要充值的号码或者账号'}
                            count={255}
                            onChange={(value) => {
                    ...
                    <List>
                        <View>
                            {calculate !== null ? <Item style={{display: "none"}}
                                                        extra={
                                                            <View>
                                                                <Text style={styles.freightPrice}>+
                                                                    ¥{calculate.pay_freight_fee}</Text>
                                                            </View>
                                                        }
                            >
                                运费
    ...
    async onCreateOrder() {
        ...
        const {cateType} = navigation.params.cateType;
        // if (!this.state.addressId) {
        //     return Toast.info("请选择收货地址");
        // }
        if (cateType !== 'member' && !this.state.message) {
            return Toast.info("请填写您要充值的号码或者账号");
        }
        const orderInfo = await buyModel.create({
            'way': 'charge',
            'cateType': navigation.params.cateType,
            'address_id': this.state.addressId,
